        ...
        try {
            ...
            if (action === "connect" && (!serverString || !password)) {
                return interaction.reply({
                    content:
                        "You must provide server information and password to connect.",
                    ephemeral: true,
                });
            }
    ...
    try {
        ...
        if (client.rcon && client.rcon.connected) {
            await interaction.editReply(
                "Already connected to a Minecraft server. Disconnect first.",
            );
            return;
        }
...
async function handleCommand(client, interaction, command) {
    ...
    try {
        if (!client.rcon || !client.rcon.connected) {
            await interaction.editReply(
                "Not connected to any Minecraft server. Use `/mc connect` first.",
            );
            return;
        }
...
async function handleStatus(client, interaction) {
    ...
    try {
        ...
        const [playerList, tps, worldInfo] = await Promise.all([
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                `Connected to ${client.mcServerInfo.host}:${client.mcServerInfo.port}`,
            )
            .addFields(
...
async function handleSay(client, interaction, message) {
    ...
    try {
        ...
        await interaction.editReply(
            `Message sent to Minecraft server: ${message}`,
        );
    } catch (error) {
